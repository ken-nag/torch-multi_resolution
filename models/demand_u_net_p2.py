...
class DemandUNet_p2(nn.Module):
    def __init__(self):
        ...
        self.ex2_channels = (1,32)
        self.encoder_channels = [(1,32), (32,32), (64,64), (64,64), (64,64)]
        self.decoder_channels = [(64,64), (128,64), (128,64), (96,32), (64,16)]
        self.last_channels = (self.decoder_channels[-1][-1], 1)
        
        self.encoder_depth = len(self.encoder_channels)
        
        self.ex2_encoder = self._encoder_block(in_channels=self.ex2_channels[0], 
                                               out_channels=self.ex2_channels[1], 
    ...
    def forward(self, xin, ex2_xin):      
        outputs = []
        ex_outputs = []
        xpad, freqs, frames = self._stride_pad(xin, stride=self.encoder_stride)
        ex2_xpad, ex2_freqs, ex2_frames = self._stride_pad(ex2_xin, stride=self.ex2_stride)
        x = xpad.unsqueeze(1)
        ex2_x = ex2_xpad.unsqueeze(1)
        
        outputs.append(x)
        ex_outputs.append(self.ex2_encoder(ex2_x))
        
        # encoder
        for i in range(len(self.encoders)):
            if i == 2:
                ex_out = self._ex_pad(ex_outputs[0], outputs[-1])
