...
class FeatExtractorBlstm_p1(nn.Module):
    def __init__(self, cfg):
        ...
        self.mix_encoder = self._encoder(channels=self.mix_channel, kernel_size=self.mix_kernel, stride=self.mix_stride)
...
if __name__ == '__main__':
    dnn_cfg = {'dnn_cfg': {'f_size': int(2048/2) + 1, 
                          ...
                          'mix_stride': (1,1),
                          'mix_channel': (30, 60),
                          'first_linear_out': int(2048/4) + 1,
                          'hidden_size': 400}}

    model = FeatExtractorBlstm_p1(dnn_cfg['dnn_cfg'])
    params = 0
